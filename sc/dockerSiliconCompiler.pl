...
sub createImage($Target)                                                                                                # Install base packages and silion compiler for local use with the correct userid number so that the docker container can write back to the local file system without running into file permission problems
 {$Target eq Local() or $Target eq GitHub() or die "Bad $Target";
...
sub createGitHub2222()                                                                                                  # Create an image for use on github  - we have to change userid number to match what github expects - creates a massive cimage which works but loads too slowly to be useful
