...
//final static boolean                        runOpenRAM = true;                                                        // Run OpenRAM to create memories for programs
  ...
  final FileNames                             traceFiles = verilogTestFolder.traceFile();                               // Verilog trace file
  ...
  final Stack<Label>                              labels;                                                               // Labels for instructions in this process
  ...
  final static Counts                        wastedReads = new Counts();                                                // Locations which pointlessly reread the same value from memory
  final static Counts                       wastedWrites = new Counts();                                                // Locations which pointlessly rewrote the same value into memory
...
//   }
  ...
  static class Counts                                                                                                   // Counts at a location identified by a string
   {final TreeMap<String,Counter> counts = new TreeMap<>();                                                             // Count at each location

    int size () {return counts.size();}                                                                                 // Number of counted locations

    void inc (String Location)
     {if (Location == null) return;                                                                                     // No location supplied - presumably because tracing information is being suppressed due to its immense size
      if (!counts.containsKey(Location)) counts.put(Location, new Counter());                                           // Only initialize if we have not seen this location before
      counts.get(Location).count++;                                                                                     // Update count
     }

    List<Map.Entry<String, Counter>> entries ()                                                                         // Locations with most counts first
     {final List<Map.Entry<String, Counter>> e = new ArrayList<>(counts.entrySet());
      e.sort((a, b) -> Integer.compare(b.getValue().count, a.getValue().count));
      return e;
     }

    public String toString()                                                                                            // Print counts
     {final StringBuilder s = new StringBuilder();

      for (Map.Entry<String, Counter> e : entries())                                                                    // Descending order of count
       {s.append(f("%4d\n%s", e.getValue().count, e.getKey()));
        break;                                                                                                          // Print just the first one - one of the ones with the maximum count
       }
      return ""+s;
     }
   }
...
//D2 Boolean values                                                                                                     // Operations on boolean values
  ...
  final class Bit                                                                                                       // A boolean value
   {boolean    i = false;                                                                                               // Value of the boolean
    ...
    Bit ie (Ops Op, Bit I) {I.S(); T(Op); new I() {void a() {ex(Op,I);} String v() {return eV(Op);}}; W(); return this;}
    ...
    String registerName(int R)                                                                                          // Register name for bit memory name from number
     {return switch (R)
       {case 0  -> bitMemory().read0Int();
        case 1  -> bitMemory().read1Int();
        case 2  -> bitMemory().read2Int();
        default -> {stop("Register must be from 0-2 not:", R); yield null;}
       };
     }

    abstract class LoadSourceOrTarget
     {LoadSourceOrTarget(String Label, Bit B, int Register, boolean LoadValue)                                          // Load source or target value via id of boolean
       {final Memory  m = bitMemory();                                                                                  // Memory to be read from
        final String mi = pV(m.readWriteIndex());                                                                       // Index
        ...
        if (LoadValue)                                                                                                  // Load value if requested
         {new I()                                                                                                       // Load source value
           {void   a() {//if (m.read0Int == (B.i ? 1 : 0)) wastedReads.inc(traceBack);                                  // Only two values so there are bound to be collisions
                        m.read0Int = B.i ? 1 : 0; jTrace(f("%8d BST2-"+Label+" "+mv+" %8d",  pc(),  B.i ? 1 : 0));}
            String v() {return                    vTrace(  "%8d BST2-"+Label+" "+mv+" %8d", "pc",   m.memory(mi));}
           };
         }

        if (LoadValue && Register > 0)                                                                                  // Load value into source register if requested to prevent subsequent load of other registers from overwriting This value,  As the target uses the common memory registers, loads of target must be done last before an instruction is executed.
         {new I()                                                                                                       // Load source value
           {void   a() {loadValue(B.i);           jTrace(f("%8d BST3-"+Label+" "+ms+" %8d",  pc(),  B.i ? 1 : 0));}
            String v() {return ms+" <= "+mv+"; "+ vTrace(  "%8d BST3-"+Label+" "+ms+" %8d", "pc",   m.memory(mi));}
           };
         }
       }
    ...
    void S ()                                                                                                           // Address source boolean value and load its value
     {new LoadSourceOrTarget("S", this, 1, true)
       {void loadValue(boolean V) {sourceBit  (V);}
       };
     }

    void T ()       {T(true);}                                                                                          // Ops that do need the target value to be loaded before instruction execution
    void T (Ops Op) {T(Op != Ops.del && Op != Ops.set);}                                                                // Ops that do not need the target value to be loaded before instruction execution

    void T (boolean LoadValue)                                                                                          // Load target index and optionally value at that index
     {new LoadSourceOrTarget("T", this, 0, LoadValue)
    ...
    String eV (Ops Op)            {return eV(Op, bitMemory().read1Int());}                                              // Dyadic boolean operation against memory
...
//D2 Integer values                                                                                                     // Operations on integer values
  ...
  final class Int                                                                                                       // An integer value
   {private int          i = 0;                                                                                         // Value of the integer
    ...
    final int       fastId;                                                                                             // Unique id for fast integer

    Int ()           {ai(); fast = false; fastId = -1; del(0);         ints().push(this);}                              // Constructors without name. Invalidate the integer. The invalidation is done in such a way as to make the instruction trace sequences for Java and Verilog match. Recall that the Verilog integers do not carry a valid flag with them as this would be a waste of resources given that the correctness of the algorithm has been already been established by successfully executing the tests associated with the Java version . The integers used in the Java version do carry a valid flag which has been helpful in validating the correctness of this implementation of the btree algorithm before handing it off to Verilog.
    Int (int I)      {ai(); fast = false; fastId = -1; ie(Ops.set, I); ints().push(this);}                              // Set the new integer to a known value at this point in the code
    Int (Int I)      {ai(); fast = false; fastId = -1; ie(Ops.set, I); ints().push(this);}                              // Set the new integer to a specified value at this point in the code
    ...
    String registerName(int R)                                                                                          // Register name for integer memory name from number
     {return switch (R)
       {case 0  -> intMemory().read0Int();
        case 1  -> intMemory().read1Int();
        case 2  -> intMemory().read2Int();
        default -> {stop("Register must be from 0-2 not:", R); yield null;}
       };
     }
    ...
    Int ex (Ops Op)                                                                                                     // Execute a monadic integer operation
     {executingCheck();
      if (Op != Ops.zero) x();                                                                                          // Check that the source of the operation exists if there is a source
      v = true;
      switch(Op)
      ...
      new ForCount(Width)
       {void body(Int Index)
         {final Int s = SourceOffset.Add(Index);
          ...
          final Int v = SourceMemory.getInt(s);                                                                         // Set source memory data value
                                     putInt(t, v);                                                                      // Save source memory data value in target memory
         }
       };
      subFinish();
      return this;
     ...
     {final Bit       r = new Bit();                                                                                    // Location at which the bit will be stored in the bit memory
      ...
      new I()                                                                                                           // Complete write
       {void   a() {if (x) bits.units[r.id] = r.i ? 1 : 0; bits.writeIntEnable        = false; jTrace(f("%8d getBit4 disable write",  currentPc()));}
        String v() {return                                 bits.writeIntEnable() + " <= 0;" +  vTrace(  "%8d getBit4 disable write", "pc"         );}
       };
      return r;
     }
     ...
     {final Int I = Index, J = Value;                                                                                   // Load the index and the value
      ...
      new I()                                                                                                           // Finish write
       {void   a() {if (units[I.i] == J.i()) wastedWrites.inc(traceBack);
        ...
        String v() {return              writeIntEnable() + " <= 0;" +  vTrace(  "%8d putInt4 Finish", "pc"         );}
       };
      return this;
     }
...
//D2 Memory references                                                                                                  // References to byte memory
    ...
    final class Ref                                                                                                     // Reference into memory
     {final Int     offset = new Int();                                                                                 // Offset of this reference in memory
      final Int offsetBits;                                                                                             // Offset of this reference in memory
...
//D2 Dump memory                                                                                                        // Dump or print memory
    ...
    String dumpJavaMemoryInDecimal ()                                                                                   // Dump java memory in decimal format
     {final int N = 10;
      final StringBuilder s = new StringBuilder();
    ...
    //s.append(f("      read0Int = %8d\n", read0Int));                                                                  // Different behavior in Verilog and Java.
      s.append(f("      read1Int = %8d\n", lui(read1Int)));
      s.append(f("      read2Int = %8d\n", lui(read2Int)));
      s.append(f("      writeInt = %8d\n", lui(writeInt)));
      s.append(f("readWriteIndex = %8d\n", readWriteIndex));
      s.append(f("writeIntEnable = %8d\n", writeIntEnable ? 1 : 0));
...
//D3 Overrides                                                                                                          // Methods that modify the behaviour of an instruction
    ...
    abstract void a ();                                                                                                 // Override to specify the action to be performed in Java by the instruction
    String        v () {return "";};                                                                                    // Override to specify the Verilog code to be generated for this instruction
...
""", "name", called(), "title", title);
  ...
  void dumpJavaMemories ()     {for(Memory m : memories()) appendJavaTrace(m.dumpJavaMemoryInDecimal());}               // Dump all the memories
...
//D2 Instruction counts                                                                                                 // Count the number of instructions in each subroutine minus the instructions supplied by called subroutines
  ...
  static void executionStatistics()                                                                                     // Execution statistics
   ...
   {final String                          name = testName();                                                            // Name of test
    ...
    String json()                                                                                                       // Json describing statistics
     {final StringBuilder s = new StringBuilder();
      ...
      s.append(f(  ", \"suppressTraceBackComments\" : \"%d\"",   suppressTraceBackComments ? 1 : 0));                   // Add trace comments to trace output to locate the point in the Java code at which the Verilog was generated - requires a lot of memory
    ...
    GenerateVerilog ()                                                                                                  // Generate the Verilog corresponding to the Java code
     {int countInstructionSets = 0;                                                                                     // Count of instructions in instruction set before we make it final
      ...
      if (!suppressTraceBackComments) printInstructionLocations();                                                      // Create a file showing the traceback at the time each instruction was created
...
""");
      ...
      if (nextFastIntId > 0) createFastInts();                                                                          // Create array of fast integers if any have been used
...
""", "memoryId", M.i(), "n", M.nameSp());
    ...
    //$fwrite(traceFile, "      read0Int = %8d\\n", {n}_read0Int);                                                      // Different behavior in Java and Verilog - in Verilog continuously assigned what ever is indexed by readWriteIndex, in Java just the last update
      $fwrite(traceFile, "      read1Int = %8d\\n", {n}_read1Int);
      $fwrite(traceFile, "      read2Int = %8d\\n", {n}_read2Int);
      $fwrite(traceFile, "      writeInt = %8d\\n", {n}_writeInt);
      $fwrite(traceFile, "readWriteIndex = %8d\\n", {n}_readWriteIndex);
      $fwrite(traceFile, "writeIntEnable = %8d\\n", {n}_writeIntEnable ? 1 : 0);
...
//D2 Instruction Matching                                                                                               // Classify instructions into blocks of identical instructions and then compressing out the duplicates to reduce code size
    ...
    void printInstructionLocations()                                                                                    // Print the location of each instruction
     {final String f = verilogTestFolder.file("InstructionLocations").txt$();                                           // Instruction to location listing file
      ...
      try
       (final BufferedWriter out = Files.newBufferedWriter(Path.of(f)))                                                 // Java's ghastly way of opening files
       {out.write(""+s);
       }
      catch(Exception e) {stop(e, fullTraceBack(e));}                                                                   // Close output file
     }
...
""", "name", b.name, "id", ""+b.id, "v", bitMemory().memory(b.id)));
      else v.append(substitute("""
      $fdisplay(traceFile, "Bit  %8d ==    %8d",        {id}, {v});
""",                 "id", ""+b.id, "v", bitMemory().memory(b.id)));
     }
...
""");
  ...
  static void test_arrayConstant(boolean Ex)
   {sayCurrentTestName();
    final int [] a = {2, 3, 5, 7, 11};
    final Program P = new Program(new Build().immediate(Ex).memory(16))
     {void code()
       {final Int i = new Int("i").set(2);
        i.S();
        final String v = new ArrayConstant(a).verilog(intMemory().read1Int(), intMemory().writeInt());
        final Int j = new Int("j");
        new I()
         {void        a() {j.targetInt(a[i.i()]); j.setValid();}
          String      v() {return v;}
          boolean trace() {return false;}
         };
        j.TW();
  ...
  static void test_arrayConstant()
   {          test_arrayConstant(true);
              test_arrayConstant(false);
   }
