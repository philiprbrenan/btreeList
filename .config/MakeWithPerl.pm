...
if ($file =~ m(\.(v|sv|tb)\Z))                                                                                         # Verilog
 {my $n = fn $file;
  ...
  if ($ext =~ m(\Atb\Z))
   {for my $e(qw(sv v))
     {if  (-e setFileExtension($file, $e))
       {$tb = $file; $ut = setFileExtension $file, $e;
        last;
       }
     }
    die "Cannot find matching .sv or .v file for $file" unless $tb;
   }
  else
   {$ut = $file; $tb = setFileExtension $file, q(tb);
#   if (!-e $tb) {die "Cannot locate matching tb file for $file"}
    $tb = '' unless -e $tb;
   }
