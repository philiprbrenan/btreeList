//----------------------------------------------------------------------------------------------------------------------
// Convert an array of integers into an OpenRAM generated read only memory
...
public class Rom extends Test                                                                                           // Create a read only memory
...
//D1 Tests                                                                                                              // Tests
  ...
  private static void test_max()
   {sayCurrentTestName();
    ...
    ok(new Rom(null).max(a), 9);
   }

  private static void test_checkArray()
   {sayCurrentTestName();
    final int [] a = {2,5,1,7,9,3};
    new Rom(a).checkArray(a);
   }

  private static void test_convertIntsToBits()
   {sayCurrentTestName();
    final int [] a = {2,5,1,7,9,3};
    ok(new Rom(null).convertIntsToBits(a, 4), "001001010001011110010011");
   }

  private static void test_convertBitsToNibbles()
   {sayCurrentTestName();
    final int [] a = {2,5,1,7,9,3};
    ok(new Rom(null).convertBitsToNibbles("001001010001011110010011", 4), "251793");
   }
...
//int wordsPerRow ()  {return sqrt(bytesPerWord() * BITS_PER_BYTE * array.length);}                                     // Words per row assuming bits occupy squares
  ...
  private static void test_genRom()
   {sayCurrentTestName();
    ...
    a.generateRom("RomPrimes", verilogTestsFolder);
   }
