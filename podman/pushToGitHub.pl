...
#-----------------------------------------------------------------------------------------------------------------------
# Build a container to run the Java code and resulting Verilog tests on GitHub without having to configure the runner each time
    ...
    steps:
      ...
      - name: Create Dockerfile
        run: |
          ...
          RUN DEBIAN_FRONTEND=noninteractive apt-get install -y git iverilog  openjdk-25-jdk-headless tree yosys podman gh
          RUN rm -rf /var/lib/apt/lists/*
...
lll "$f  Work flow for create build container: $repo";

my $P = fpf qq(podman), fne($0);
my $F = writeFileUsingSavedToken $user, $repo, $P, $0;                                                                  # Upload this file
lll "$f $P";
